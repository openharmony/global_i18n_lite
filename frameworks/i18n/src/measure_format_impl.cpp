    ...
    I18nStatus &status)
{
    ...
    int concatenate = 0;
    if (isNumberFirst) {
        concatenate = sprintf_s(result, MAX_MEASURE_FORMAT_LENGTH, pattern.c_str(), formattedNumber.c_str(),
            formattedUnit.c_str());
    } else {
        concatenate = sprintf_s(result, MAX_MEASURE_FORMAT_LENGTH, pattern.c_str(), formattedUnit.c_str(),
